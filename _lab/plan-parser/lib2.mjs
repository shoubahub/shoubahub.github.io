...
/* ⚠ والخريطة تبنى من كل عملية رسم على حدة: كلمة انقسمت عمليتين («األسبو» + «ع» — لغتي العربية للمتوسط)
   لا يقع معطوبها في الخريطة فيبقى مبعثرا، فسقطت ثماني صفحات الى القراءة الضوئية (2026-09-12). فبعد الخريطة
   اصلاح ثابت لرابطة اللام مع الهمزة والمد: «ا + أ/إ/آ + ل» لا يقع في عربية سليمة ابدا، فصوابه «ال + أ/إ/آ».
   (لا «لال»: تقع في كلمات سليمة كـ«خلال») */
const LAM_HAMZA = s => s.replace(/ا([أإآ])ل/g, 'ال$1');
/* ⚠ وخط ارقامه في خانات حروف النغمة (U+02F9 = ٠ …): خطتا الاحصاء للثانوي «( ˺ – ˻ )» ⟵ «( 1 – 2 )»، و«ن ˼˹» ⟵
   «ن 30» في شرط العينة الكبيرة — فالخانة ناقص U+02F9 هو الرقم. وخانات النغمة لا تقع في نص مقرر عربي ابدا
   (2026-09-13؛ رأيت ٠–٤، والحد U+02FF ⟵ ٦ لان ما بعده علامات مركبة لا يؤمن تحويلها) */
const TONE_DIGITS = s => s.replace(/[˹-˿]/g, c => String(c.charCodeAt(0) - 0x2F9));
export function makeFixer(map){
  const keys = [...map.keys()].filter(k => k.length >= 3).sort((a,b)=>b.length-a.length);
  return (s) => TONE_DIGITS(fixed(s));
  function fixed(s){
