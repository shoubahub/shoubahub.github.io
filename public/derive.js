...
(function () {
  ...
  /* ── مراقبة الشعبة (2026-09-19، قرار المستخدم: «بذكر يوم المراقبة دون تفاصيل الاماكن») ──────────
     ايام الاسبوع التي على الشعبة فيها مراقبة — يذكر بها جدول اليوم في اللوحة عند الفرص. لا نوب معلمين
     ولا اماكن: نوبة كل معلم شأن يومي يتغير، وذكر اليوم هو ما يحتاجه رئيس الشعبة (d.duty: «اليوم» ⟵ true) */
  S.dutyDays = function () {
    var m = S.data().duty || {};
    return (SHOUBA_REF.days || []).filter(function (day) { return !!m[day]; });
  };
  S.setDutyDay = function (day, on) {
    var d = S.data(), m = d.duty || (d.duty = {});
    if (on) m[day] = true; else delete m[day];
    S.save();
  };
